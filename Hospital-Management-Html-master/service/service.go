	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			...
			newpatient := models.Customer{
				...
				PatientHealth:   []models.PatientEntry{},
			}

			inserted, err := config.Customer_ProfileCollection.InsertOne(ctx, newpatient)
			if err != nil {
				log.Fatal(err)
...
func Updatepatient(profile models.PatientEntry, patientID string) error {
	var ctx context.Context

	// Create a filter to match the customer with the specified patientID
	filter := bson.M{
		"patientid": patientID,
	}

	// Create an update that uses the $push operator to add a new element to the "patienthealth" array
	update := bson.M{
		"$push": bson.M{
			"patienthealth": profile,
		},
	}

	// Perform the update with UpdateOne
	result, err := config.Customer_ProfileCollection.UpdateOne(ctx, filter, update)
	if err != nil {
...
func Getgrapgdata(id string) ([]models.PatientEntry, error) {
	var patient models.Customer
	filter := bson.M{"patientid": id}
	err := config.Customer_ProfileCollection.FindOne(context.Background(), filter).Decode(&patient)
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			// Handle the case where no documents are found
			return patient.PatientHealth, fmt.Errorf("No data found for the given ID: %s", id)
		}

		// Handle other errors
		return patient.PatientHealth, err
	}
fmt.Println(patient.PatientHealth)
	return patient.PatientHealth, nil
